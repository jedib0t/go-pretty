...
func TestNewListWriter(t *testing.T) {
	...
	lw.SetStyle(ListStyleConnectedBold)
	assert.NotNil(t, lw.Style())
	assert.Equal(t, ListStyleConnectedBold, *lw.Style())
}
...
func TestList_SetStyle(t *testing.T) {
	...
	list.SetStyle(ListStyleDefault)
	assert.NotNil(t, list.Style())
	assert.Equal(t, &ListStyleDefault, list.Style())
}

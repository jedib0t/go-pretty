...
func TestTable_sortRows_WithName(t *testing.T) {
	...
	assert.Equal(t, []int{0, 1, 2, 3}, table.sortRows(table.rows))
	...
	assert.Equal(t, []int{3, 2, 1, 0}, table.sortRows(table.rows))

	// sort by First Name, Last Name
	table.SortBy([]SortBy{{Name: "First Name", Mode: Asc}, {Name: "Last Name", Mode: Asc}})
	assert.Equal(t, []int{0, 2, 1, 3}, table.sortRows(table.rows))
	...
	assert.Equal(t, []int{3, 1, 2, 0}, table.sortRows(table.rows))
	...
	assert.Equal(t, []int{3, 2, 0, 1}, table.sortRows(table.rows))
	...
	assert.Equal(t, []int{1, 0, 2, 3}, table.sortRows(table.rows))
	...
	assert.Equal(t, []int{2, 0, 1, 3}, table.sortRows(table.rows))

	table.SortBy([]SortBy{{Name: "Salary", Mode: DscNumeric}})
	assert.Equal(t, []int{3, 0, 1, 2}, table.sortRows(table.rows))
